...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class GameState
    {
        ...
        public bool TrySetProperties(Hashtable props, out bool changed, out string debugMessage)
        {
            changed = false;
        ...
        private bool UpdateProperties(Hashtable props)
        {
            ...
            if (props.ContainsKey((byte)GameParameter.IsVisible))
            {
                object value = props[(byte)GameParameter.IsVisible];
                if (value is bool)
                {
            ...
            if (props.ContainsKey((byte)GameParameter.IsOpen))
            {
                object value = props[(byte)GameParameter.IsOpen];
                if (value is bool)
                {
            ...
            if (props.ContainsKey((byte)GameParameter.MaxPlayer))
            {
                object value = props[(byte)GameParameter.MaxPlayer];
                if (value is byte)
                {
                    this.MaxPlayer = (byte)value;
                    changed = true;
                }
            ...
            if (props.ContainsKey((byte)GameParameter.Properties))
            {
                object value = props[(byte)GameParameter.Properties];
                var collection = value as object[];
                if (collection != null)
                {
                    var lobbyProperties = new HashSet<object>(collection);
